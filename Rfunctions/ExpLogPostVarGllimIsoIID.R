ExpLogPostVarGllimIsoIID<-function(modg,matz,R){
  ###############################################################################
  ...
  # posterior logvariances in the GLLiM IID isotropic case for an 
  ...
  for(k in 1:K){
    ...
    tmpM[,,k]<-Aa[,,k]%*%t(t(covstarR[,,k]))%*%t(Aa[,,k])/(Sigmaa[1,1,k]^2)
  }

PreCompDistFullParIID1D<-function(modg,R,y,matz,wthr=0){
  ## PAS FINI not goof
  #################################################################################
  ...
  for(k in 1:K){
    tmpk<-t(Aa[,,k])%*%t(t(ca[,k]))+t(t(ba[,k]))
    tmpyDR<-t(rowSums(yDR-tmpk[,1]))
    # tmpM pourrait etre calculer avant but is reused via tmpM anyway
    tmpMk<-t(invSigmaa[,,k])%*%t(Aa[,,k])%*%t(t(covstarR[,,k]))%*%Aa[,,k]%*%t(invSigmaa[,,k])
    logpiyk<--0.5*sum(quad.diag(t(invSigmaa[,,k]),(yDR-tmpk[,1]))) +0.5*quad.form(tmpMk,t(tmpyDR))-0.5*logdetVR[k]
  ...
  Pimatz<- foreach(i=1:M, .combine=rbind) %dopar% {
    ...
    for (k in 1:K){
      tmpzDR<-t(rowSums(matzDRi-t(t(tmp[,k]))  ))
      logpizk<--0.5*sum(quad.diag(t(invSigmaa[,,k]),(matzDRi-t(t(tmp[,k])) ))) +0.5*quad.form(tmpM[,,k],t(tmpzDR))-0.5*logdetVR[k]
  ...
  for (k in leftky){
    # FULL case
    Asybs<-cbind(Asybs,t(t(covstarR[,,k]))%*%(Aa[,,k]%*%t(invSigmaa[,,k])%*%t(t(rowSums(yDR-t(t(ba[,k]))  ))) + t(invGammaa[,,k])%*%t(t(ca[,k])) ) )
  }
  # L x dimay  (y)
  postmeany=Asybs
  ...
  listdist<- foreach (i=1:M, .combine=cbind) %dopar% {
    ...
    for (k in leftkz){
      # ISOTROPIC case
      Asybs<-cbind(Asybs,t(t(covstarR[,,k]))%*%(Aa[,,k]%*%t(invSigmaa[,,k])%*%t(t(rowSums(t(t(matzDR[,,i]))-t(t(ba[,k])) ) ) ) + invGammaa[,,k]%*%t(t(ca[,k])) ) )
    }
    # L x dimaz  (y)
    postmeanz=Asybs

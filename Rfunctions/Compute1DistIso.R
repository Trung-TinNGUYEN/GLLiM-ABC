Compute1DistIso<-function(modg,y,z,wthr=0){
#################################################################################
# Case L> 1 and one distance at a time    (for use with doPar or parallelisation)
# ie when the parameter is multidimensional (see Compute1DistL1 for the
# univariate case:
...
#################################################################################
  ...
  #### step2 : Compute MW2 and L2 between gllim posterior given y and 
  ####           the gllim posterior given z
  MW2dist<-was2mix(mixy,mixz)
  L2dist<- L2normal(mixy,mixz)

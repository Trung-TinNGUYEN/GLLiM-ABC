PreCompDistFullParIIDMargin<-function(modg,R,y,matz,imarg,wthr=0){
  #################################################################################
  # Version of PreComDistFullPar with distances computed on the marginal mixtures
  #
  ...
  ## imarg and the index margins eg c(1,2)
  #
  redL<-length(imarg)  #redL<L
  ...
  for(k in 1:K){
    ...
    logpiyk<--0.5*sum(quad.diag(invSigmaa[,,k],(yDR-tmpk[,1]))) +0.5*quad.form(tmpMk,t(tmpyDR))-0.5*logdetVR[k]
  ...
  # End compute posterir weights: not changed by marginalisations
  
  postcovy<-array(0,c(redL,redL,dimay))
  postmeany<-matrix(0,redL,dimay)
  
  postcovy[,,1:dimay]=covstarR[imarg,imarg,leftky]
  ...
  for (ii in 1:dimay) {
    sigma1<-postcovy[,,ii]
    for (jj in 1:K){
      sigma2<-covstarR[imarg,imarg,jj]
      E2 <- eigen(sigma2)
      V2 <- E2$vectors
      U2 <- solve(V2)
      D2 <- diag(E2$values) 
  ...
  for (k in leftky){
    # FULL case
    Asybs<-cbind(Asybs,t(t(covstarR[,,k]))%*%(t(Aa[,,k])%*%invSigmaa[,,k]%*%t(t(rowSums(yDR-ba[,k]))) + invGammaa[,,k]%*%t(t(ca[,k])) ) )
  }
  # L x dimay  (y) : ici redL xdimay
  postmeany=Asybs[imarg,]
  # problem with networkflow when K=1
  # to avoid we duplicate the mixture in this case
  
  if (dimay==1) {
    postmeany<-cbind(postmeany,postmeany)
    temp=array(0,c(redL,redL,2))
    temp[,,1]<-postcovy
    temp[,,2]<-postcovy
    postcovy<-temp
    Piy=c(0.5,0.5)
  ...
  listdist<- foreach (i=1:M, .combine=cbind) %dopar% {
    ...
    postcovz<-array(0,c(redL,redL,dimaz))
    postmeanz<-matrix(0,redL,dimaz)
    
    postcovz[,,1:dimaz]=covstarR[imarg,imarg,leftkz]
    
    Asybs<-NULL
    for (k in leftkz){
      # ISOTROPIC case
      Asybs<-cbind(Asybs,t(t(covstarR[,,k]))%*%(t(Aa[,,k])%*%invSigmaa[,,k]%*%t(t(rowSums(t(t(matzDR[,,i]))-ba[,k]))) + invGammaa[,,k]%*%t(t(ca[,k])) ) )
    }
    # L x dimaz  (y) : here redl x dimaz
    postmeanz=Asybs[imarg,]

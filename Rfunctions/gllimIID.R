gllimIID = function(tapp,yapp,iR,in_K,in_r=NULL,maxiter=150,Lw=0,cstr=NULL,verb=0,in_theta=NULL){
  ...
  ExpectationZIID = function(tapp,yapp,yappR,iR,th,verb){
    ...
    for (k in 1:K){
      ...
      sumRpdf<-0
      for (i in i:iR){sumRpdf<-sumRpdf+loggausspdf(yappR[,,i],muyk,covyk)}
  ...
  MaximizationIID = function(tapp,yapp,yappR, iR, r,muw,Sw,cstr,verb){
    ...
    for (k in 1:K){
      ...
      #        }
      #else{
      for (iirep in 1:iR){yk_bartemp<-yk_bartemp + rowSums(sweep(yappR[,,iirep],2,rk,"*"))/rk_bar[k]}
      #  }
